...
class DenoisingNet_MLP_NST_Orth(torch.nn.Module):
    def __init__(
        self,
        patch_size,
        D_in,
        ...
        super(DenoisingNet_MLP_NST_Orth, self).__init__()
        self.patch_size = patch_size
        ...
        #self.Dict = torch.nn.Parameter(Dict_init)
        # Parameterize Dict as U * S * VT
        self.U = torch.nn.Parameter(U_init)
        geotorch.orthogonal(self, "U")              # Enforce unitary matrix
        self.U.data = U_init                        # Reinitialize

        self.S = torch.nn.Parameter(S_init)         # 

        self.VT = torch.nn.Parameter(VT_init)       
        geotorch.grassmannian(self, "VT")           # Enforce tight frame
        self.VT.data = VT_init                      # Reinitialize
    ...
    def forward(self, x):                                                       # EXPLANATIONS
        ...
        #y = torch.matmul(unfold, self.Dict)                                     # Compute D^T y   (transposed)
        y = torch.matmul(unfold, self.U)                                       # Compute D^\dagger y (transposed) = VT' *S^-1 *U' (tranposed)
        y = y * self.S.pow(-1)
        y = torch.matmul(y, self.VT)

        S = self.Identity - (1 / self.c) * self.VT.t().mm(self.VT)              # Compute I - 1/L * D^T D
        S = S.t()                                                               # Nic: isn't it symmetrical? Why need to transpose? Checked with np.linalg.norm(S.cpu().detach().numpy() - S.t().cpu().detach().numpy())
        ...
        #x_pred = torch.matmul(z, self.Dict.t())                                 # Final approximated reconstruction = D * x
        x_pred = torch.matmul(z, self.VT.t())                                   # Final approximated reconstruction = D * x = U *S*VT*x
        x_pred = x_pred * self.S
        x_pred = torch.matmul(x_pred, self.U.t())
...
class DenoisingNet_MLP_NST_Orth2(torch.nn.Module):
    def __init__(
        self,
        patch_size,
        D_in,
        ...
        super(DenoisingNet_MLP_NST_Orth2, self).__init__()
        self.patch_size = patch_size
    ...
    def forward(self, x):                                                       # EXPLANATIONS
        ...
        x_pred = torch.clamp(x_pred, min=self.min_v, max=self.max_v)            # Saturate at -1 and +1, probably because input images a like this
        x_pred = self.w_1 * x_pred                       # w size = 64            # Weight each of the output pixel of the patch. These are learned weights!
        x_pred = x_pred.transpose(1, 2)                # size 18 x 14641 x 64   
        ...
        x_pred = torch.clamp(x_pred, min=self.min_v, max=self.max_v)            # Saturate at -1 and +1, probably because input images a like this
        x_pred = self.w_2 * x_pred                       # w size = 64            # Weight each of the output pixel of the patch. These are learned weights!
        x_pred = x_pred.transpose(1, 2)                # size 18 x 14641 x 64   
